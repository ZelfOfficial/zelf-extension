...
export class OptimismService {
    ...
    private readonly _chainConfigs = {
        mainnet: {
            ...
            rpcUrls: [environment.optimismRpc.mainnet],
            nativeCurrency: {
                decimals: 18,
                name: "Ether",
                symbol: "ETH",
    ...
    constructor(
        ...
        this._web3Promise = this._rpcProvider.getWeb3("optimism");
    }
    ...
    private async _getTransactionCost(
        ...
        try {
            ...
            console.error("Error in Optimism getTransactionCost:", error);
    ...
    async calculateTransactionFees(
        ...
        try {
            ...
            console.error("Error calculating Optimism transaction fees:", error);
    ...
    async getWalletDetails(address?: string): Promise<any> {
        const url = `${this._baseUrl}/api/optimism/address/${address}`;
        ...
        try {
            ...
            console.error("Exception in Optimism getWalletDetails:", error);
    ...
    async requestTransactionDetails(address: string, transactionHash: string): Promise<{ data: any }> {
        const url = `${this._baseUrl}/api/optimism/address/${address}/transaction/${transactionHash}`;
    ...
    async requestTransactionHistory(address: string, pagination: { page: number; show?: number }): Promise<any> {
        const url = `${this._baseUrl}/api/optimism/address/${address}/transactions`;
        ...
        try {
            ...
            console.error("Exception in Optimism requestTransactionHistory:", error);
    ...
    async sendTransaction(params: TransactionParams): Promise<TransactionResult> {
        try {
            if (!params.privateKey) throw new Error("Private key is required for Optimism transactions");

            const provider = await this._rpcProvider.getEthersProvider("optimism");
            ...
            console.error("Error sending Optimism transaction:", error);
            throw error;
        }
    }
}
