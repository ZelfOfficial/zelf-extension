...
export class BaseService {
    ...
    private readonly _chainConfigs = {
        mainnet: {
            ...
            rpcUrls: [environment.baseRpc.mainnet],
            nativeCurrency: {
                decimals: 18,
                name: "Ether",
                symbol: "ETH",
    ...
    constructor(
        ...
        this._web3Promise = this._rpcProvider.getWeb3("base");
    }
    ...
    private async _getTransactionCost(
        ...
        try {
            ...
            console.error("Error in Base getTransactionCost:", error);
    ...
    async calculateTransactionFees(
        ...
        try {
            ...
            console.error("Error calculating Base transaction fees:", error);
    ...
    async getWalletDetails(address?: string): Promise<any> {
        const url = `${this._baseUrl}/api/base/address/${address}`;
        ...
        try {
            ...
            console.error("Exception in Base getWalletDetails:", error);
    ...
    async requestTransactionDetails(address: string, transactionHash: string): Promise<{ data: any }> {
        const url = `${this._baseUrl}/api/base/address/${address}/transaction/${transactionHash}`;
    ...
    async requestTransactionHistory(address: string, pagination: { page: number; show?: number }): Promise<any> {
        const url = `${this._baseUrl}/api/base/address/${address}/transactions`;
        ...
        try {
            ...
            console.error("Exception in Base requestTransactionHistory:", error);
    ...
    async sendTransaction(params: TransactionParams): Promise<TransactionResult> {
        try {
            if (!params.privateKey) throw new Error("Private key is required for Base transactions");

            const provider = await this._rpcProvider.getEthersProvider("base");
